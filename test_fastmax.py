...
fastmax = FASTMultiHeadAttention(use_custom_gradient=False)
torch.no_grad()
...
b = 64 # batch
h = 16 # head
d = 64 # dimension per head (i.e. model dimension/h)
...
# b = 2 # batch
# h = 3 # head
# d = 2 # dimension per head (i.e. model dimension/h)
# # n changes from 10^strt to 10^endd. The number of test points are count
# rep = 1
# count = 1
# strt = 0.5 # log scale
...
for i in np.logspace(strt, endd, count):
  ...
  for ii in range(rep):
    ...
    q = torch.normal(0,1,[b,h,int(i),d],device=torch.device('cuda')).contiguous()
    k = torch.normal(0,1,[b,h,int(i),d],device=torch.device('cuda')).contiguous()
    v = torch.normal(0,1,[b,h,int(i),d],device=torch.device('cuda')).contiguous()
...
#     for ii in range(rep):
#         torch.cuda.empty_cache()
#         q = torch.normal(0,1,[b,h,int(i),d],device=torch.device('cuda'),dtype=torch.float16)
#         k = torch.normal(0,1,[b,h,int(i),d],device=torch.device('cuda'),dtype=torch.float16)
#         v = torch.normal(0,1,[b,h,int(i),d],device=torch.device('cuda'),dtype=torch.float16)

#         start_time = time.time()
#         # e = softmax(q,k,v)
#         e = F.scaled_dot_product_attention(q, k, v, dropout_p = 0.1)
#         # print(e[0][0][0][0])
#         cuda.synchronize()
#         end_time = time.time()
